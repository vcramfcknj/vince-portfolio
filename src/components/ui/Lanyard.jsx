...
// Preload assets to avoid suspense jank
...
export default function Lanyard({
  ...
  return (
    <div style={{ position: 'relative', zIndex: 1, width: '100%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'flex-start' }}>
      <Canvas
...
function Band({
  ...
  useFrame((state, delta) => {
    ...
    try {
      ...
      if (!j1.current.lerped) j1.current.lerped = new THREE.Vector3().copy(j1Trans);
      if (!j2.current.lerped) j2.current.lerped = new THREE.Vector3().copy(j2Trans);

      const d1 = Math.max(0.1, Math.min(1, j1.current.lerped.distanceTo(j1Trans)));
      const d2 = Math.max(0.1, Math.min(1, j2.current.lerped.distanceTo(j2Trans)));

      j1.current.lerped.lerp(j1Trans, delta * (minSpeed + d1 * (maxSpeed - minSpeed)));
      j2.current.lerped.lerp(j2Trans, delta * (minSpeed + d2 * (maxSpeed - minSpeed)));
      ...
      // Ignore transient physics initialization ticks
    }
  });
  ...
  return (
    <>
      <group position={[0, 4.4, 0]}>
        ...
        <RigidBody position={[0.4, -0.2, 0]} ref={j1} {...segmentProps}>
          <BallCollider args={[0.1]} />
        </RigidBody>
        <RigidBody position={[0.8, -0.4, 0]} ref={j2} {...segmentProps}>
          <BallCollider args={[0.1]} />
        </RigidBody>
        <RigidBody position={[1.2, -0.6, 0]} ref={j3} {...segmentProps}>
          <BallCollider args={[0.1]} />
        </RigidBody>
        <RigidBody position={[1.6, -0.8, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
